...
class Shell:
    ...
    def __init__(self, slot_dir=None):
        ...
        self.version = "1.0"
        self.musicHome = os.path.abspath("music")
        self.variables = {
            "MAIN": self.main,
    ...
    class Network:
        def __init__(self):
            self.shellNet = "AvalonShell"       # shell net
            self.version = "0.1.0"              # shell net version
            self.backboneAPI = Shell()          # apis

        def ping(self, webOrUrl: str, specBytes: bytes = b"", count: int = 1):
            """
            Ping a URL over HTTP.
            webOrUrl: URL to ping
            specBytes: optional bytes to send via POST
            count: how many times to ping
            """
            try:
                # Parse the hostname
                hostname = urlparse(webOrUrl).hostname
                if hostname is None:
                    print(f"Invalid URL: {webOrUrl}")
                    return

                # Resolve IP
                ip = socket.gethostbyname(hostname)
                print(f"{hostname} resolves to IP: {ip}")
                ...
                for i in range(count):
                    ...
                    # GET request
                    try:
                        response = requests.get(webOrUrl, timeout=5)
                        elapsed = (time.time() - start_time) * 1000  # ms
                        print(f"[GET] {webOrUrl} - Status: {response.status_code}, Time: {elapsed:.2f}ms")
                    except requests.RequestException as e:
                        print(f"[GET] Failed: {e}")

                    # POST request (if bytes provided)
                    if specBytes:
                        try:
                            start_post = time.time()
                            post_response = requests.post(webOrUrl, data=specBytes, timeout=5)
                            elapsed_post = (time.time() - start_post) * 1000
                            print(f"[POST] Sent {len(specBytes)} bytes - Status: {post_response.status_code}, Time: {elapsed_post:.2f}ms")
                        except requests.RequestException as e:
        ...
        def curl(self, url: str, saveFile=False):
            try:
                response = requests.get(url, timeout=5)
                response.raise_for_status()
                print(response.text)  # prints page content

                if saveFile:
                    parsed = urlparse(url)
                    filename = parsed.path.split('/')[-1] or "downloaded_file"
                    pathfile = os.path.join(self.backboneAPI.crr_dir, filename)

                    with open(pathfile, "wb") as f:
                        f.write(response.content)  # save raw bytes
                    print(f"File saved to: {pathfile}")

            except requests.exceptions.RequestException as e:
                print(f"{Fore.LIGHTRED_EX}Error fetching URL: {e}{Fore.RESET}")
            except Exception as e1:
                print(f"{Fore.LIGHTRED_EX}Error executing curl: {e1}{Fore.RESET}")
    ...
    def returnerCMD(self):
        ...
        try:
            ...
            elif cmd in ("listdir", "ls", "dir", "rt.ls", "rt.dir", "rt.listdir"):
                if len(args) >= 1:
            ...
            elif cmd in ("rename", "ren", "rt.rename", "rt.ren"):
                if len(args) >= 2:
                    returned += f"{args[0]} - {args[1]}"
                else:
                    returned += f"Missing tokens"
            ...
            elif cmd in ("slink", "sh.link", "shell.link"):
                if len(args) >= 2:
                    returned += f"{args[0]}: {" ".join(args[1:])}"
                else:
                    returned += "Missing Tokens"
            ...
            elif cmd in ("trustedinstaller", "trusted", "trustmebro"):
                returned += "TrustedInstaller"
            ...
            elif cmd in ("wait", "sleep", "task.wait"):
                if len(args) >= 1:
                    returned += f"{float(args[0])}"
                else:
                    returned += "Missing tokens"
...
"""
            ...
            elif cmd in ("smusic", "bg.smusic"):
                returned += "stopMusic"

            elif cmd in ("pmusic", "bg.pmusic"):
                returned += "pauseMusic"

            elif cmd in ("upmusic", "bg.upmusic"):
                returned += "unpauseMusic"
    ...
    def music(self, file:str=None, loop:bool=True):
        pygame.mixer.init()
        buffer = os.path.join(self.musicHome, file)

        if not os.path.exists(buffer):
            print(f"{Fore.RED}File: {buffer} doesn't exist{Fore.RESET}")
            return
        elif not os.path.isfile(buffer):
            print(f"{Fore.RED}Target: {buffer} isn't a file{Fore.RESET}")
            return
        ...
        self.music_paused = False
        audio = eyed3.load(buffer)
        if audio.tag is not None:
            artist = audio.tag.artist or "Unknown Artist"
            title = audio.tag.title or "Unknown Title"
            album = audio.tag.album or "Unknown Album"
        else:
            print("Unknown song(Music)")
        print(f"{Fore.GREEN}Currently playing: {title} - Artist: {artist} - Album: {album}")
    ...
    def AdminShell(self):
        ...
        if osname == "Windows":
            if not ctypes.windll.shell32.IsUserAnAdmin():
                print("AvalonReports: Relaunching AvalonShell with admin privileges...")
                ret = ctypes.windll.shell32.ShellExecuteW(
                    None, "runas", sys.executable, " ".join(sys.argv), None, 1
                )
                if ret <= 32:
                    print("Admin relaunch cancelled")
                sys.exit()
        else:  # Linux/macOS
            if os.geteuid() != 0:
                print("AvalonReports: Relaunching AvalonShell with admin privileges...")
                sub.call(["sudo", sys.executable] + sys.argv)
                sys.exit()

        # If we’re here, we’re admin
        print("AvalonReports: AvalonShell is now running with Admin privileges.")
        print("AvalonReports: CAUTION: Deleting OS files can result in a full reinstall")


    
    
    def TrustedShell(self):
        if platform.system() != "Windows":
            print("TrustedShell only supports Windows.")
            return

        # Full path to your DLL     TrustedInstallerAdjustments.dll
        dll_path = os.path.abspath("TrustedInstallerAdjustments.dll")
        if not os.path.exists(dll_path):
            print(f"{self.fcmd}AvalonReports: DLL not found at {dll_path}{Fore.RESET}")
            return

        # Load DLL
        try:
            ti_dll = ctypes.WinDLL(dll_path)
        except Exception as e:
            print(f"{self.fcmd}AvalonReports: Failed to load DLL: {e}{Fore.RESET}")
            return

        # Load impersonation functions
        impersonate = ti_dll.ImpersonateTrustedInstaller
        impersonate.restype = ctypes.wintypes.BOOL

        revert = ti_dll.RevertFromTrustedInstaller
        revert.restype = ctypes.wintypes.BOOL

        # Attempt impersonation
        if impersonate():
            print(f"{self.fcmd}AvalonReports: Shell is now impersonating TrustedInstaller privileges.{Fore.RESET}")
            try:
                # Spawn a child Python shell under impersonation
                sub.run([sys.executable, "-i"])
            except Exception as e:
                print(f"{self.fcmd}AvalonReports: Error running TrustedShell: {e}{Fore.RESET}")
            finally:
                # Always revert to original privileges
                if revert():
                    print(f"{self.fcmd}AvalonReports: Shell reverted to original privileges.{Fore.RESET}")
                else:
                    print(f"{self.fcmd}AvalonReports: Failed to revert privileges!{Fore.RESET}")
        else:
            print(f"{self.fcmd}AvalonReports: Failed to impersonate TrustedInstaller.{Fore.RESET}")
    ...
    def OSHost(self, host="", live=True):
        ...
        try:
            ...
            if live:
                proc = sub.Popen(host_cmd, stdout=sub.PIPE, stderr=sub.STDOUT,
                                text=True, bufsize=1, universal_newlines=True, shell=True)
                for line in proc.stdout:
                    print(line, end="")  # stream live output
                proc.wait()
            else:
                result = sub.run(host_cmd, capture_output=True, text=True, shell=True)
                if result.stdout:
                    print(result.stdout)
                if result.stderr:
...
{Fore.RESET}""")     
    ...
    def sleeper(self, times:int|float, bg=False):
        import threading
        def sleep():
            try:
                times = float(times)
            except ValueError:
                print(f"{Fore.LIGHTRED_EX}Decimal(Float) conversion failed: {times}{Fore.RESET}")
            time.sleep(times)
        if bg:
            threading.Thread(sleep).start()
        else:
            sleep()
    ...
    def help(self):
        ...
        commands = [
            ...
            cmd(f"{self.fcmd}listdir/ls/dir/rt.listdir/rt.ls/rt.dir{Fore.RESET} {self.fargs}<Extra Args: -f(file only), -d(dir only), -l(detailed), -r(reversed), -b(sort by bytes) -p(full path) -h(human readable size, only work with -l).(These are optional)>{Fore.RESET}", "List all items in the current directory", "|"),
            ...
            cmd(f"{self.fcmd}oshost{Fore.RESET} {self.fargs}<anything>{Fore.RESET}", "Run a host system command", "|"),
            ...
            cmd(f"{self.fcmd}rename/ren/rt.rename/rt.ren{Fore.RESET} {self.fargs}<FileName> <NewName>{Fore.RESET}", "Rename a file", "|"),
            ...
            cmd(f"{self.fcmd}trusted/trustedshell/trustmebro{Fore.RESET}",
                "Run AvalonShell with TrustedInstaller privileges",
                "|"
            ),
            ...
            cmd(f"{self.fcmd}music/bg.music{Fore.RESET} {self.fargs}<fileInside the music folder> <extraArgs[-lf for no looping]>{Fore.RESET}",
                "play bg music",
                "|"
            ),

            cmd(f"{self.fcmd}smusic/bg.smusic{Fore.RESET}",
                "stop bg music",
                "|"
            ),

            cmd(f"{self.fcmd}pmusic/bg.pmusic{Fore.RESET}",
                "pause bg music",
                "|"
            ),

            cmd(f"{self.fcmd}upmusic/bg.upmusic{Fore.RESET}",
                "unpause bg music",
                "|"
            ),
    ...
    def parse(self, user_input: str):
        ...
        for each in splitedLines:
            ...
            try:
                ...
                if cmd == "help":
                ...
                elif cmd in ("trusted", "trustedshell", "trustmebro"):
                    self.TrustedShell()
                    return True
                    ...
                    if not args:
                        print(f"{self.ferr}Usage: oshost <anythingHere>{Fore.RESET}")
                        return False
                    else:
                        live = True
                        lift = 0
                        ...
                        if len(args) >= 1:
                            self.OSHost(" ".join(args[lift:]), live)
                        else:
                            self.OSHost(args[0], "", live)
                        return True
                ...
                elif cmd in ("ls", "listdir", "dir", "rt.ls", "rt.dir", "rt.listdir"):
                    reverse = False
                    detailed = False
                    sort_size = False
                    only_cfiles = False
                ...
                elif cmd in ("slink", "shell.link", "sh.link"):
                    if not args:
                        ...
                        if len(args) >= 3:
                            HOSTSHELLcmd = " ".join(args[:-1])
                        self.Slink(otherShell, shellCMD, HOSTSHELLcmd)
                        return True
                    

                elif cmd in ("sleep", "wait", "task.wait"):
                    if len(args) >= 1:
                        lift = 0
                        background = False
                        for arg in args:
                            if arg == "-b":
                                lift += 1
                                background = True
                        self.sleeper(args[lift], background)
                        return True
                    
                    else:
                        print("Usage: sleep/wait/task.wait <time-decimal-or-integere>")
                        return False
                    ...
                    if len(args) >= 1:
                        ...
                        threading.Thread(self.music(args[0], falseLoop)).start()
                    else:
                        print("Usage: music <.mp3File> <extraArgs[-lf for no looping]>")
                    ...
                    if len(args) >= 1:
                        ...
                        print(f"{self.fsuccess}# {Fore.RESET}")
                        return True
                

                else:
                    path = os.path.join(self.crr_dir, cmd)
                    system = platform.system()
                    ...
                    try:
                        if system == "Windows":
                            ...
                            st = os.stat(path)
                            os.chmod(path, st.st_mode | st.S_IEXEC)
                            sub.run([path, *args])
                        else:
                            print(f"{self.ferr}Unsupported platform: {system}")
                            return False
                    except Exception as e:
                        print(f"{self.ferr}Error executing command: {e}")
                        return False
            except Exception as e:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if not line or line.startswith("#"):  # ignore empty lines and comments
                        continue
                    if self.bgProccessAll:
                        threading.Thread(self.parse, args=line)
                    else:
        ...
        try:
            while True:
                try:
                    ...
                    if len(parts) >= 1:  # check user input, not accumulated commands
                        cmd = parts[0].lower()
                    ...
                    if ">>" in self.totalCMDs:
                        ...
                        if cmd == "paste":
                            if self.bgProccessAll:
                                threading.Thread(self.parse, args=self.paste())
                            else:
                                self.parse(self.totalCMDs)
                        elif cmd == "runfile":
                            file = args[0]
                            ...
                            if len(args) >= 1 and args[0] in ("false", "true", "False", "True", "1", "0"):
                                self.bgProccessAll = bool(args[0])
                                print(f"Bg Cmds parameter have been set to: {args[0]}")
                            else:
                                print(f"Usage: as.pro <1 or 0, true or false>")
                            self.totalCMDs = ""
                        else:
                            if self.returnerCMD() == "Unknown":
                                print(f"{Fore.LIGHTRED_EX}Unknown cmd: {cmd}{Fore.RESET}")
                            else:
                                if self.bgProccessAll:
                                    threading.Thread(target=self.parse, args=(self.totalCMDs,)).start()
                                else:
                                    self.parse(self.totalCMDs)
